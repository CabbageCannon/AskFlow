...
# ANTHROPIC和OPENA都是模型原生的搜索能力，而TAVILY是额外的搜索工具
...
def remove_up_to_last_ai_message(
    ...
    for i in range(len(messages) - 1, -1, -1):
        if isinstance(messages[i], AIMessage):
            ...
            truncated_messages = messages[:i]
            if final_human_message is not None:
                truncated_messages.append(final_human_message)
